...
class SetDirectControlStopClientNode(CovviBaseClientNode):
    def __init__(self, service: str = '', node_name: str = 'covvi_set_direct_control_stop_client_node', **kwargs):
        super().__init__(service=service, node_name=node_name, **kwargs)
        full_service_name_SetDirectControlStop = self._get_full_ros2_name('SetDirectControlStop', service=service)
...
def main(args: Iterable[Any] | None = None) -> None:
    ...
    covvi_set_direct_control_stop_client_node = SetDirectControlStopClientNode(service=service)
    rclpy.spin(covvi_set_direct_control_stop_client_node)
    covvi_set_direct_control_stop_client_node.destroy_node()
    rclpy.shutdown()
