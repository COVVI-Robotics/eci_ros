...
class DisableAllRealtimeCfgClientNode(CovviBaseClientNode):
    def __init__(self, service: str = '', node_name: str = 'covvi_disable_all_realtime_cfg_client_node', **kwargs):
        super().__init__(service=service, node_name=node_name, **kwargs)
        full_service_name_DisableAllRealtimeCfg = self._get_full_ros2_name('DisableAllRealtimeCfg', service=service)
...
def main(args: Iterable[Any] | None = None) -> None:
    ...
    covvi_disable_all_realtime_cfg_client_node = DisableAllRealtimeCfgClientNode(service=service)
    rclpy.spin(covvi_disable_all_realtime_cfg_client_node)
    covvi_disable_all_realtime_cfg_client_node.destroy_node()
    rclpy.shutdown()
